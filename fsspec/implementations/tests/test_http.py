...
import fsspec.asyn
import fsspec.utils
from fsspec.dircache import MemoryDirCache, FileDirCache, CacheType
...
def test_list_cache(server):
    ...
    assert h.listings_cache_options == {"cache_type": CacheType.MEMORY, "expiry_time": None}
    assert issubclass(h.dircache.__class__, MemoryDirCache)
    out = h.glob(server + "/index/*")
    assert out == [server + "/index/realfile"]
...
def test_list_cache_memory(server, expiry_time):
    h = fsspec.filesystem(
        "http", listings_cache_options={"cache_type": "memory", "expiry_time": expiry_time}
    )
    assert h.listings_cache_options == {"cache_type": CacheType.MEMORY, "expiry_time": expiry_time}
    assert issubclass(h.dircache.__class__, MemoryDirCache)
    start = time.time()
    out = h.glob(server + "/index/*")
...
def test_list_cache_file(server, tmp_path, expiry_time):
    h = fsspec.filesystem(
        "http", listings_cache_options={"cache_type": "file", "expiry_time": expiry_time, "directory": tmp_path}
    )
    ...
    assert issubclass(h.dircache.__class__, FileDirCache)
