...
def test_file_existence_checks(repo):
    ...
    # I would not have expected that ls on a file works. I would not be surprised
    # if this behavior is changed in the future or for other fsspec implementations.
    assert fs.ls("inner/file1")

    assert not fs.lexists("non-existing-file")
    assert not fs.exists("non-existing-file")

    # I would not have been surprised if these also raised exceptions, similar to "info".
